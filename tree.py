...
class DecisionTree:
    def __init__(self, num_features: int, max_depth: int=None) -> None:
    ...
    def fit(self, X: Tensor, y: Tensor) -> None:
        ...
        self._fit_recursive(self.root, X, indices, y, set(), 0)

    def _fit_recursive(self, node: Node, X: Tensor, indices: Tensor, y: Tensor, used_features: set, depth=None) -> None:
        classes, counts = torch.unique(y[indices], return_counts=True)
        parent_counts = torch.bincount(y[indices], minlength=self.n_classes)
        ...
        feature_class_counts = self._compute_counts(X, indices, y[indices])

        gains = torch.zeros(len(sampled_features))
        for i, f in enumerate(sampled_features):
            gains[i] = self._information_gain(f, y[indices], feature_class_counts, parent_counts)

        if torch.max(gains).item() <= 1e-9:
            node.label = classes[torch.argmax(counts)].item()
            return
        ...
        # presence of word
        node.add_child(1)
        split_mask = self._get_split_mask(X, indices, feature)
        right_indices = indices[split_mask]
        self._fit_recursive(node.children[1], X, right_indices, y, new_used_features, depth=next_depth)

        # absence of word
        node.add_child(0)
        left_indices = indices[~split_mask]
        self._fit_recursive(node.children[0], X, left_indices, y, new_used_features, depth=next_depth)

    def _get_split_mask(self, X: Tensor, X_indices: Tensor, feature: int) -> Tensor:
        crow = X.crow_indices()
        col = X.col_indices()

        starts = crow[X_indices]
        ends = crow[X_indices + 1]
        mask = torch.zeros(len(X_indices), dtype=torch.bool)

        for i, (s, e) in enumerate(zip(starts, ends)):
            row_cols = col[s:e]
            if torch.any(row_cols == feature):
                mask[i] = True
        return mask

    def _compute_counts(self, X: Tensor, indices: Tensor, y: Tensor) -> Tensor:
        feature_class_counts = torch.zeros(
            (self.num_features, self.n_classes),
            dtype=torch.long,
            device=X.device
        )
        ...
        for c in range(self.n_classes):
            class_mask = (y == c)
            active_rows_in_class = indices[class_mask]

            if active_rows_in_class.numel() == 0:
                continue

            starts = crow[active_rows_in_class]
            ends = crow[active_rows_in_class + 1]

            for s, e in zip(starts, ends):
                row_features = cols[s:e]
                feature_class_counts[:, c].index_add_(0, row_features, torch.ones_like(row_features, dtype=torch.long))
    ...
    def predict(self, X: Tensor) -> Tensor:
        ...
        preds = torch.zeros(num_samples, dtype=torch.long, device=X.device)
        ...
        for i in range(num_samples):
            ...
            while node.feature is not None and node.children:
                ...
                row_features = col[row_start:row_end]
                found = False
                if torch.any(row_features == feature_idx):
                    found = True

                idx = 1 if found else 0
...
#         x_indices = torch.arange(0, len(X))
#         self._fit_recursive(self.root, X, x_indices, y, set(), 0)
#
#     def _fit_recursive(self, node: Node, X: Tensor, X_indices: Tensor, y: Tensor, used_features: set, depth=None) -> None:
